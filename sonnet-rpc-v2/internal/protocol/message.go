...
// DecodeHeaderLen parses headerLen from byte slice
func DecodeHeaderLen(data []byte) uint32 {
	return binary.BigEndian.Uint32(data)
}

// DecodeBodyLen parses bodyLen from byte slice
func DecodeBodyLen(data []byte) uint32 {
	return binary.BigEndian.Uint32(data)
}

// DecodeBytes decodes complete Message from byte array (for sticky packets)
