...
func NewClient(addr string, svcName string, etcdCli *client_v3.Client) (*Client, error) {
	...
	grpcClient := pb.NewlarkCacheClient(conn)
...
func (c *Client) Get(group, key string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get value from lark_cache_v2: %v", err)
	}
...
func (c *Client) Delete(group, key string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to delete value from lark_cache_v2: %v", err)
	}
...
func (c *Client) Set(ctx context.Context, group, key string, value []byte) error {
	...
	if err != nil {
		return fmt.Errorf("failed to set value to lark_cache_v2: %v", err)
	}
