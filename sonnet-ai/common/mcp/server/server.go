...
func NewMCPServer() *server.MCPServer {
	...
	mcpServer.AddTool(
		mcp.NewTool(
			"get_weather",
			mcp.WithDescription("Get weather info for a specified city"),
			mcp.WithString(
				"city",
				mcp.Description("City name, e.g. Beijing, Shanghai"),
				mcp.Required(),
			),
		),
...
// StartServer starts MCP server
// httpAddr: HTTP server listening address (e.g. ":8080")
